...
class RendererDynLib : Renderer {
 private:
  ...
  void (*prepareIslandPtr)(void* bufferTmp, float x1, float y1, int maxBridges);
 public:
  RendererDynLib(std::string path){
      ...
      prepareIslandPtr = (void (*)(void* bufferTmp, float x1, float y1, int maxBridges))dlsym(handle, "prepareIsland");
  }
  ~RendererDynLib(){
      dlclose(handle);
      delete buffer;
  ...
  void prepareIsland(float x1, float y1, int maxBridges) {prepareIslandPtr(buffer, x1, y1, maxBridges);}
};
